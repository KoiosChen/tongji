...
var mApp = function() {
    ...
    */
    var initAlerts = function() {
        // init bootstrap popover
        $('body').on('click', '[data-close_gate=alert]', function() {
        ...
        */
        alert: function(options) {
            options = $.extend(true, {
                ...
                reset: true, // close_gate all previouse alerts first
                focus: true, // auto scroll to the alert after shown
                closeInSeconds: 0, // auto close_gate after defined seconds
                icon: "" // put icon before the message
            }, options);

            var id = mUtil.getUniqueID("App_alert");

            var html = '<div id="' + id + '" class="custom-alerts alert alert-' + options.type + ' fade in">' + (options.close ? '<button type="button" class="close_gate" data-dismiss="alert" aria-hidden="true"></button>' : '') + (options.icon !== "" ? '<i class="fa-lg fa fa-' + options.icon + '"></i>  ' : '') + options.message + '</div>';
    ...
    $.fn.mDropdown = function (options) {
        ...
        var Plugin = {
            ...
            disableClose: function () {
                element.on('click', '.m-dropdown--disable-close_gate, .mCSB_1_scrollbar', function (e) {
                    e.preventDefault();
                    e.stopPropagation();
                });
            },
            ...
            show: function() {
                ...
                element.addClass('m-dropdown--open_gate');
            ...
            hideHovered: function(force) {
                if (force) {
                    ...
                    Plugin.clearHovered();        
                    element.removeClass('m-dropdown--open_gate');
                    dropdown.open = false;
             ...
             */
            hideOpened: function() {
                $('.m-dropdown.m-dropdown--open_gate').each(function() {
    ...
    $.fn.mMenu = function(options) {
        ...
        var Plugin = {
            ...
            reset: function() {
                ...
                menu.find('.m-menu__item--open_gate:not(.m-menu__item--expanded)').removeClass('m-menu__item--open_gate');
            },
            ...
            */
            ...
            handleSubmenuDropdownClick: function(e) {
                ...
                if (item.hasClass('m-menu__item--hover') == false) {
                    item.addClass('m-menu__item--open_gate-dropdown');
                    Plugin.showSubmenuDropdown(item);
                } else {
                    item.removeClass('m-menu__item--open_gate-dropdown');
                    Plugin.hideSubmenuDropdown(item, true);
                }
            ...
            /**
             * Handles submenu dropdown close_gate on link click
             * @returns {mMenu}
             */
            handleSubmenuDropdownClose: function(e, el) {
                ...
                if (shown.length > 0 && el.hasClass('m-menu__toggle') === false && el.find('.m-menu__toggle').length === 0) {
                    // close_gate opened dropdown menus
                    shown.each(function() {
            ...
            handleSubmenuAccordion: function(e, el) {
                ...
                if (item.closest('.m-menu__item').hasClass('m-menu__item--open_gate-always')) {
                    return;
                }
                ...
                if (submenu.length > 0) {
                    ...
                    if (li.hasClass('m-menu__item--open_gate') === false) {
                        // hide other accordions
                        if (menu.options.accordion.expandAll === false) {
                            var closables = item.closest('.m-menu__nav, .m-menu__subnav').find('> .m-menu__item.m-menu__item--open_gate.m-menu__item--submenu:not(.m-menu__item--expanded):not(.m-menu__item--open_gate-always)');
                        ...
                        if (hasClosables) {
                            ...
                            li.addClass('m-menu__item--open_gate');
                        } else {
                            submenu.slideDown(speed, function() {
                                Plugin.scrollToItem(item);
                            });
                        ...
                        li.removeClass('m-menu__item--open_gate');
                    }
                }
            },     
            ...
            resetActiveItem: function(item) {
                ...
                // close_gate open_gate submenus
                if (menu.options.accordion.expandAll === false) {
                    element.find('.m-menu__item--open_gate').each(function() {
                        $(this).removeClass('m-menu__item--open');
                    });
                }
            },
...
$.notifyDefaults({
	...
	'<button type="button" aria-hidden="true" class="close_gate" data-notify="dismiss"></button>' +
