...
class Converter
  module LessConversion
    ...
    def read_files(path, files)
      ...
      if File.directory?(path)
        files.each do |name|
          ...
          contents[name] = File.read(total_path, mode: 'rb') if File.exists?(total_path)
        end
      end
      contents
    end
    ...
    def process_stylesheet_assets
      ...
      files   = read_files(Dir.pwd, bootstrap_less_files)
      ...
      files.each do |name, file|
        ...
        path = File.join save_to, File.basename(name.sub(/\.less$/, '.scss'))
        save_file(path, file)
    ...
    def bootstrap_less_files
      @bootstrap_less_files ||= Dir["less/*.less"]
    end
